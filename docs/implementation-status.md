...
| API Connections | `/workspace`; `/api/integrations`, `/api/integrations/oauth` | `provider_connections`, `integration_oauth_states`; encrypted credentials require `INTEGRATION_ENCRYPTION_KEY`; provider app registrations and Access identity | Integrated local connection center: 12 searchable/clickable provider cards; Twilio, Stripe, and Resend credential-verification routes; OAuth start/callback and encrypted token storage for Google, Meta, LinkedIn, X, and Reddit. Coverage: provider readiness, status secrecy, OAuth fail-closed behavior, and Playwright UI. No real external account was authorized in this environment. See [docs/provider-connections.md](provider-connections.md). | Configure provider apps and Worker secrets, then verify each provider with a test account. Implement delivery, publishing, billing, sync, and DNS operations as separate tested slices. |
...
- `npm test`: 25 tests passed. D1-backed Worker/API and Playwright coverage exercises booking lifecycle, tenant and manager scopes, concurrency, the connection status endpoint, OAuth fail-closed behavior without app secrets, and the provider page's search, detail dialogs, Stripe connect form, and any-domain copy. Install Chromium with `npx playwright install chromium` before running the suite.
- Focused ESLint: 0 errors and two pre-existing unused-variable warnings in `public/views.js` (`renderCompanies`, `dashboardTenantId`). Full ESLint still reports the existing `app/foundation/page.tsx:61` setState-in-effect error.
- `npm run build`: passed; includes `/api/appointments`, `/api/integrations`, and `/api/integrations/oauth`.
