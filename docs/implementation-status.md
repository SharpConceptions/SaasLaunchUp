...
| Calendars | `/workspace` Calendar, Booking pages, Availability, Appointments; `/api/appointments` | `appointment_availability`, `appointments`, `appointment_reminder_jobs`, CRM contacts/opportunities, activities, audit events; no external calendar/email/SMS provider | Backend exists; the UI and API now support availability, CRM-linked booking, reschedule, cancel, overlap protection, and dry-run reminders. Evidence: [app/api/appointments/route.ts](../app/api/appointments/route.ts), [lib/appointments.ts](../lib/appointments.ts), [public/views.js](../public/views.js), [db/schema.ts](../db/schema.ts), [drizzle/0016_robust_veda.sql](../drizzle/0016_robust_veda.sql), [tests/appointments.test.mjs](../tests/appointments.test.mjs). Automated local tests pass; authenticated Worker/D1 walkthrough is still required before Integrated or Verified. | Walk the flow using a deployed tenant, then add external calendar sync as a separate slice. |
